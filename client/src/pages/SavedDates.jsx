...
const SavedDates = () => {
  ...
  useEffect(() => {
    const fetchSavedDates = async () => {
      try {
        ...
        console.log("Saved dates:", response.data);
        dispatch({
          type: UPDATE_TOKEN,
          payload: response.headers.authorization.split(" ").pop().trim(),
        });
  ...
  const updatedDateRedux = (date) => {
    ...
    console.log("Date object going into redux:", date);
    dispatch({
      type: SAVED_DATE,
      payload: date,
    });
          ...
          <ul className="card">
            {savedDates.map((date) => (
              <Link
                to={{
                  pathname: "/saved-date",
                  state: { foodPlace: date.foodPlace, activity: date.activity },
                }}
                key={date._id}
